...
public class Jaguar extends Predator {
     ...
     */
    public Jaguar(boolean randomAge, Field field, Location location,Simulator sim){
        super(randomAge, field, location,sim);
        f = field;
        setBreedingAge(5);
        setMaxAge(60);
        setBreedingProbability(0.70);
        setMaxLitterSize(6);
        setPreyFoodValue(9);
        setSleepTime(16);
        setSleepDuration(3);
    ...
    protected void giveBirth(List<Animal> newPredators)
    {
        ...
        for(int b = 0; b < births && free.size() > 0; b++) {
            Location loc = free.remove(0);
                Predator young = new Jaguar(false, field, loc, getSimulator());
