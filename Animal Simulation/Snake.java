...
public class Snake extends Predator {
     ...
     */
    public Snake(boolean randomAge, Field field, Location location, Simulator sim){
        super(randomAge, field, location, sim);
        f = field;
        setBreedingAge(15);
        setMaxAge(60);
        setBreedingProbability(0.7);
        setMaxLitterSize(3);
        setPreyFoodValue(9);
        setSleepTime(4);
        setSleepDuration(19);
    ...
    protected void giveBirth(List<Animal> newPredators)
    {
        ...
        for(int b = 0; b < births && free.size() > 0; b++) {
            Location loc = free.remove(0);
                Predator young = new Snake(false, field, loc, getSimulator());
